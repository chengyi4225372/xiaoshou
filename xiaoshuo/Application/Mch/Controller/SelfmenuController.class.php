<?php
namespace Mch\Controller;
use Think\Controller;
class SelfmenuController extends CommonController {
   
	public function index(){
		$list = $this -> _get_menu_list();
		$this -> assign('list', $list);
		$this -> display();
	}
	
	
	public function edit(){
		if(IS_POST){
			// 不能选择自己作为父菜单
			if(isset($_GET['id']) && intval($_POST['pid']) == intval($_GET['id'])){
				$this -> error('不能选择自己作为父菜单');
			}
			$_POST['mch'] = $this->mch['id'];
			
		}
		$parents = M('selfmenu') -> where(array('pid'=>0,'mch'=>$this->mch['id'])) -> select();
		$this -> assign('parents', $parents);
		$this->_edit('selfmenu');
	}
	
	
	// 发布自定义菜单
	public function send(){
		if(IS_POST){
			$list = $this -> _get_menu_list();
			$data = array();
			foreach($list as $v){
				$tmp = array(
					'name' => $v['name'],
					'type' => $v['type']
				);
				if($v['type'] == 'view'){
					$tmp['url'] = $v['extra'];
				}else{
					$tmp['key'] = $v['extra'];
				}
					
				// 顶级菜单
				if($v['pid'] == 0){
					$data[] = $tmp;
				}else{
					$data[count($data)-1]['sub_button'][] = $tmp;
				}
			}
					
			$dd = new \Common\Util\ddwechat($this -> _mp);
			$accesstoken = $dd -> getaccesstoken();
			$rs = $dd -> createmenu($dd -> jsonencode(array('button' => $data)), $accesstoken);
			if(!$rs){
				$this -> error("操作失败!".$dd -> errmsg);
			}else{
				$this -> success('操作成功！');
				exit;
			}
		}else{
			$this->error('非法请求！');
		}
	}
	
	
	
	// 取得所有自定义菜单
	private function _get_menu_list(){
		$list = $this -> _get_list('selfmenu', array('mch'=>$this->mch['id']), 'sort desc');
		// 将数据转为树形结构
		$tree = array();
		foreach($list as &$v){
			if($v['pid'] == 0){
				$tree[] = $v;
				// 取得该顶级菜单的所有子菜单
				foreach($list as $val){
					if($val['pid'] == $v['id']){
						$tree[] = $val;
					}
				}
			}
		}
		return $tree;
	}
	
}